...
def capture_cpu_status():
	# cmd = " top -d 1 -n 3 -b -p %d | awk '{line[NR]=$0} END {for(i=NR-1;i<=NR;i++) print line[i]}' " % self.pid_id
	# cmd = " top -d 1 -n 3 -b -p %d | awk '{line[NR]=$9} END {print line[NR]}' " % os.getpid()
	cmd = "sar -u 1 5"
	p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE)
	p.wait()
	if p.poll() == 0:
		...
		if len(out) == 0:
			...
			cpu_used_status = re.findall(r"\d+\.?\d*",str(out))[-1] + "%"
