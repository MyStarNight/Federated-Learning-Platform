...
class AutoCheck:
	...
	ring_key = {'situation': {'flag_of_ok_device_sum': 0, 'system_is_running': False},
				'device': [['192.168.0.104', 0.0, 0.0], ['192.168.0.100', 0.0, 0.0], ['192.168.0.101', 0.0, 0.0]],
		...
		while True:
			...
			if flag_of_ok_device_sum == Nums_of_devices and not system_is_running:
				...
				if del_one_ip and del_one_ip[0] == _key_ring['server'][0]:
					...
					cpu_status = cst.capture_cpu_status()
					print("cpu use status is : %s" % cpu_status)
					if cpu_status >= 90:
						cmd = 'python3 infer_thread.py --source bus.jpg'
		...
		for i, c in enumerate(ip_of_devices):
			if c[0] == self.local_ip:  # found!
				if if_calculate:
					...
					c[1] = cst.test_calculate_time()
					c[2] = cst.test_band_width(standard_ip)
				i += 1  # next one
				i %= Nums_of_devices
